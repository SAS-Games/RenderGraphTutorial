Shader "Custom/VirtualDepthSpriteURP"
{
    Properties
    {
        [PerRendererData]
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                for (int sampleIndex = 0; sampleIndex < MAX_DEPTH_SAMPLES; ++sampleIndex)
                {
                    ...
                    half layerWeight = (half)_VirtualAlphas[sampleIndex];
                   ...
                   // What is the exact XYZ position at that intersection?
                    ...
                    //Sample the virtual sprite. We only care about alpha because all virtual copies use the same final color.
                    half spriteAlpha = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, textureUV).a;
                    if (spriteAlpha <= 0.001h)
                        continue;

                    // Apply artist-authored AlphaByDepth. This has already been evaluated by C#.
                    half layerAlpha = spriteAlpha * layerWeight;

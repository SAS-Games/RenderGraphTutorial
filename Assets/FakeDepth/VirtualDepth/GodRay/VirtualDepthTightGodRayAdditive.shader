Shader "Custom/URP/Virtual Depth/Tight Mesh/God Ray Additive"
{
    Properties
    {
        [PerRendererData]
    ...
    SubShader
    {
        ...
        Pass
        {
            Name "VirtualDepthTightGodRayAdditive"
            ...
            {
                Varyings output;
                float3 proxyPositionOS = CalculateVirtualDepthTightProxyPositionOS(
                        input.uv.x,
                        _LayerOffsetPerDepth.xy);
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                [unroll]
                for (int godRayLayerIndex = 0; godRayLayerIndex < VIRTUAL_DEPTH_MAX_LAYER_COUNT; ++godRayLayerIndex)
                {
                    if (godRayLayerIndex >= activeLayerCount)
                        break;

                    float layerDepth = _VirtualLayerDepths[godRayLayerIndex];
                    half layerOpacity = _VirtualLayerOpacities[godRayLayerIndex];
                    if (layerOpacity <= 0.0001h)
                        continue;

                    half layerMask = SampleVirtualDepthLayerMask(
                            viewRay,
                            layerDepth,
                            _LayerOffsetPerDepth.xy);
                    if (layerMask <= 0.001h)
                    ...
                    accumulatedLight += layerMask * layerOpacity;

Shader "Hidden/Chapter9/MaskDistortion/Composite"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float2 chromaticOffset = normalize(offset + 0.00001) * _ChromaticAberrationPixels * _SourceTexelSize.xy;

                float2 sampleUv = saturate(uv + offset);
                half4 center = SAMPLE_TEXTURE2D_X(_MaskDistortionSourceTexture, sampler_LinearClamp, sampleUv);

                if (_ChromaticAberrationPixels > 0.0001)
                {
                    half red = SAMPLE_TEXTURE2D_X(_MaskDistortionSourceTexture, sampler_LinearClamp, saturate(sampleUv + chromaticOffset)).r;
                    half blue = SAMPLE_TEXTURE2D_X(_MaskDistortionSourceTexture, sampler_LinearClamp, saturate(sampleUv - chromaticOffset)).b;

...
It is not a blur. Blur averages neighboring pixels to remove detail; distortion keeps the scene detail but samples it from animated, offset screen positions so the image appears to bend or refract. The two effects can be used independently or combined.
...
- `Assets/RenderTextureFeature/Core/MaskedEffect/ObjectMask.mat`: shared material used by the mask producer.
- `Assets/RenderTextureFeature/Core/MaskedEffect/InvisibleMaskProxy.mat`: shared material used by the example's invisible proxy mesh.
- `MaskDistortionRenderer.asset`: renderer configured with the mask producer followed by the distortion feature.
- `MaskDistortion.unity`: standalone example scene that uses renderer index `8`.
...
Add a mask output to `ObjectsToRenderTextureFeature` before configuring the distortion feature:

| Setting | Value |
| --- | --- |
| `Texture Name` | `_MaskDistortionMask`, or another unique name |
| `Material` | `ObjectMask` |
| `Material Pass Index` | `-1` |
| `Render Pass Event` | `AfterRenderingOpaques` |
| `Render Pass Input` | `Depth` |
| `Render Queue Upper Bound` | `2499` for opaque mask objects, `5000` for transparent proxies |
...
Keep `Layer Mask` set to `Everything` and select effect objects with `Render Layer Mask = Effect Mask Primary`. The same texture name must then be assigned to `MaskDistortionFeature > Distortion Settings > Mask Texture Name`.

...
- `MaskHaloMask.shader`: flat white mask shader.
- `MaskHaloMask.mat`: mask material assigned to the mask output.
- `MaskHalo.unity`: example scene using `Halo_Renderer`.
...
| `Wrap Mode` | `Clamp` |
| `Layer Mask` | `CharacterHalo` |
| `Depth` | Enabled |
| `Write Depth` | Disabled |
| `Depth Compare` | `LessEqual` |

Put the character renderer on the `CharacterHalo` layer. If changing the character's gameplay layer is undesirable, use child proxy renderers on `CharacterHalo` that follow the same bones or transforms.
...
1. Confirm the camera uses `Halo_Renderer`.

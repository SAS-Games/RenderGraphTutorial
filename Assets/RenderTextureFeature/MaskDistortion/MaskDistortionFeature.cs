...
public class MaskDistortionFeature : ScriptableRendererFeature
{
    private const string CompositeShaderName =
        "Hidden/RenderTextureFeature/MaskDistortion/Composite";

    [Tooltip("Base name used for profiling markers and generated render pass names.")]
    public string ProfilingName = "Mask Distortion";

    [Tooltip("Material that uses Hidden/RenderTextureFeature/MaskDistortion/Composite. Assign the included MaskDistortionComposite material for build-safe shader references.")]
    ...
    public class Settings
    {
        ...
        [Range(0.0f, 1.0f)]
        public float MaskSoftness = 0.08f;
        ...
        ref RenderingData renderingData)
    {
        ...
        _sourceSnapshotPass.Setup(
            $"{ProfilingName} Source",
            DistortionSettings.RenderPassEvent);

        _distortionPass.Setup(
            ProfilingName,
            DistortionSettings,
            _materialCache.Material,
            _sourceSnapshotPass.SnapshotTextureId);
    ...
    private static bool ShouldEnqueue(Settings settings)
    {
        return settings != null &&
               ...
               (settings.DistortionStrengthPixels > 0.0f ||
                settings.ChromaticAberrationPixels > 0.0f ||
                settings.TintStrength > 0.0f);
    }
    ...
    private class MaskDistortionPass : ScriptableRenderPass
    {
        private static readonly int SourceTextureId =
            Shader.PropertyToID("_MaskDistortionSourceTexture");

        private static readonly int SourceTexelSizeId =
            Shader.PropertyToID("_SourceTexelSize");

        private static readonly int DistortionStrengthPixelsId =
            Shader.PropertyToID("_DistortionStrengthPixels");

        private static readonly int DistortionFrequencyId =
            Shader.PropertyToID("_DistortionFrequency");

        private static readonly int DistortionSpeedId =
            Shader.PropertyToID("_DistortionSpeed");

        private static readonly int ChromaticAberrationPixelsId =
            Shader.PropertyToID("_ChromaticAberrationPixels");

        private static readonly int MaskThresholdId =
            Shader.PropertyToID("_MaskThreshold");
            ...
            ContextContainer frameData)
        {
            ...
            TextureHandle sourceTexture = GetSourceTexture(
                frameData,
                resourceData,
            ...
            passData.DistortionStrengthPixels =
                Mathf.Clamp(_settings.DistortionStrengthPixels, 0.0f, 32.0f);
            passData.DistortionFrequency =
                Mathf.Clamp(_settings.DistortionFrequency, 0.1f, 80.0f);
            passData.DistortionSpeed =
                Mathf.Clamp(_settings.DistortionSpeed, -10.0f, 10.0f);
            passData.ChromaticAberrationPixels =
                Mathf.Clamp(_settings.ChromaticAberrationPixels, 0.0f, 8.0f);
            ...
            passData.TimeOffset = Application.isPlaying
                ? Time.time
        ...
        private TextureHandle GetSourceTexture(
            ContextContainer frameData,
            UniversalResourceData resourceData,
            out Vector4 sourceTexelSize)
        {
            if (FrameTextureRegistry.TryGet(
                    frameData,
                    out FrameTextureRegistry textureRegistry))
            {
                if (textureRegistry.TryGetTexture(
                        _sourceTextureId,
            ...
            sourceTexelSize = MaskedEffectRenderGraphUtility.CreateTexelSize(
                descriptor.width,
                descriptor.height);
            ...
            RasterGraphContext context)
        {
            ...
            data.Material.SetFloat(
                DistortionStrengthPixelsId,
                data.DistortionStrengthPixels);
            data.Material.SetFloat(
                DistortionFrequencyId,
                data.DistortionFrequency);
            data.Material.SetFloat(DistortionSpeedId, data.DistortionSpeed);
            data.Material.SetFloat(
                ChromaticAberrationPixelsId,
                data.ChromaticAberrationPixels);

...
internal sealed class LayerBlurPass : ScriptableRenderPass
{
    ...
    private sealed class LayerRuntime
    {
        ...
        public void Setup(
            string name,
            LayerBlurFeature.Settings settings,
            LayerBlurMaterialSet materials)
        {
            Name = name;
            Settings = settings;
            Materials = materials;
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
    {
        ...
        for (int i = 0; i < _activeLayerCount; i++)
        {
            ...
            AddCompositePass(
                renderGraph,
                $"{layer.Name} Composite",
                resourceData.activeColorTexture,
                sourceColor,
                layer.BlurredTexture,
                layer.MaskTexture,
                layer.Settings,
                layer.Materials.Composite.Material);
        }
    }
    ...
    private void BuildBlurGroups(ContextContainer frameData, TextureHandle sourceColor)
    {
        ...
        {
            LayerRuntime layer = _layers[i];
            layer.HasMask = layer.MaskResolver.TryResolve(
                frameData,
            ...
            group.MaxIterations = Mathf.Max(
                group.MaxIterations,
                Mathf.Clamp(layer.Settings.Iterations, 1, 4));
            layer.BlurGroupIndex = groupIndex;
        }
    }

    private int FindOrCreateBlurGroup(
        int downsample,
        float radius,
        LayerBlurMaterialSet materials)
    {
        ...
        TextureHandle sourceColor)
    {
        ...
        {
            BlurGroup group = _blurGroups[groupIndex];
            RenderTextureDescriptor descriptor = CreateBlurDescriptor(
                cameraDescriptor,
                group.Downsample);
            ...
            {
                TextureHandle result = CreateBlurTexture(
                    renderGraph,
                    descriptor,
                    groupIndex,
                    $"Level {iteration}");
                ...
                AddBlurPass(
                    renderGraph,
                    $"{_profilingName} Chain {groupIndex + 1} Horizontal {iteration}",
                    currentSource,
                    ping,
                    group.Materials.Horizontal.Material,
                    Vector2.right,
                    texelSize,
                    group.Radius,
                ...
                AddBlurPass(
                    renderGraph,
                    $"{_profilingName} Chain {groupIndex + 1} Vertical {iteration}",
                    ping,
                    result,
                    group.Materials.Vertical.Material,
                    Vector2.up,
                    texelSize,
                    group.Radius,
    ...
    private void AssignBlurResult(int groupIndex, int iteration, TextureHandle result)
    {
        ...
        {
            LayerRuntime layer = _layers[i];
            if (layer.BlurGroupIndex == groupIndex &&
                Mathf.Clamp(layer.Settings.Iterations, 1, 4) == iteration)
            {
    ...
    private TextureHandle GetSourceColor(
        ...
        UniversalResourceData resourceData)
    {
        ...
        return textureRegistry.TryGetTexture(_sourceTextureId, out TextureHandle sourceColor, out _)
            ? sourceColor
            : resourceData.activeColorTexture;
    }

    private static RenderTextureDescriptor CreateBlurDescriptor(
        RenderTextureDescriptor descriptor,
        int downsample)
    {

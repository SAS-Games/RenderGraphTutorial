...
| `Render Layer Mask` | `Effect Mask Primary`, `Effect Mask Secondary`, or another explicitly configured rendering-layer bit |
...
Use rendering-layer bits for effect or strength buckets. The included project provides:

```text
Effect Mask Primary
Effect Mask Secondary
```

Reuse `Effect Mask Primary` in scenes that need only one mask. Reserve different bits for groups that must be captured simultaneously by the same renderer.
...
The included `FrostedGlassBlur` scene uses the dedicated `FrostedGlassBlurRenderer` with the mask output and blur entry below already configured.
...
Do not assign `ObjectMask` directly to the visible window mesh. `ObjectMask` is the override material used by `ObjectsToRenderTextureFeature` when it creates the offscreen mask. The visible window should keep its normal glass material.

Setup steps:

1. Use the shared `Effect Mask Primary` URP rendering layer.
2. Keep your visible window mesh on its normal GameObject layer and keep its normal transparent glass material.
3. Create a second mesh or quad that matches the glass shape. Name it `FrostedGlassBlurProxy`.
4. Add `Effect Mask Primary` to the proxy renderer's `Rendering Layer Mask`.
5. Assign `InvisibleMaskProxy` to the proxy mesh renderer.
6. Keep the proxy on its normal GameObject layer. The proxy material is invisible, so the player still will not see it.
7. Add a mask output in `ObjectsToRenderTextureFeature`.
8. Add one blur entry in `LayerBlurFeature`.

Scene object setup:

| GameObject | Rendering Layer | Mesh Renderer Material | Enabled when window is closed | Enabled when window is open |
| --- | --- | --- | --- | --- |
| `Window_Glass` | Default only | Your normal glass material | Yes | Usually yes, or animated open |
| `FrostedGlassBlurProxy` | `Effect Mask Primary` | `InvisibleMaskProxy` | Yes | No, or animated open with the window |
...
- If the proxy itself becomes visible in the camera, check that its Mesh Renderer material is `InvisibleMaskProxy`, not `ObjectMask`.
...
1. Use the shared `Effect Mask Primary` rendering layer.
2. Add that bit to the shield mesh or shield proxy renderer.
...
1. Use the shared `Effect Mask Primary` rendering layer.
2. Add that bit to the cloaked character renderer or silhouette proxy.
...
1. Use the shared `Effect Mask Primary` rendering layer.
2. Add that bit to the portal surface renderer.
...
3. Add `Effect Mask Primary` to that proxy renderer.
...
1. Use `Effect Mask Primary`, `Effect Mask Secondary`, and configure a third rendering-layer bit for the third simultaneous depth group.
2. Add the corresponding bit to each object or proxy renderer.
...
| Render Layer Mask | Texture Name |
| --- | --- |
| `Effect Mask Primary` | `_NearBlurMask` |
| `Effect Mask Secondary` | `_MidBlurMask` |
| Third configured bit | `_FarBlurMask` |
...
- Use proxy shapes when the visible object's silhouette should differ from the blur region. Rendering-layer selection itself does not affect gameplay or physics.
...
1. Use the shared `Effect Mask Primary` rendering layer.
2. Add that bit to the object to hide or its proxy renderer.
...
1. Use the shared `Effect Mask Primary` rendering layer.
2. Add that bit to the water or glass proxy renderer.
...
3. Add `Effect Mask Primary` to the proxy renderer.
...
1. Use the shared `Effect Mask Primary` rendering layer.
2. Add that bit to the panel background renderer or matching proxy renderer.
...
3. Add `Effect Mask Primary` to the proxy renderer.
4. Add a mask output for `_HeatBlurMask`.
5. Add a cheap subtle blur entry.
...
1. Use `Effect Mask Primary` and `Effect Mask Secondary`.
2. Add the appropriate bit to non-important object renderers or proxy regions.
3. Add two mask outputs.
4. Add two blur entries.
5. Keep both effect bits off the main gameplay subject's renderers.

Mask output values:

| Render Layer Mask | Texture Name |
| --- | --- |
| `Effect Mask Primary` | `_CinematicSoftBlurMask` |
| `Effect Mask Secondary` | `_CinematicHeavyBlurMask` |
...
1. Use `Effect Mask Primary` and `Effect Mask Secondary`; add another rendering-layer bit only when a third simultaneous group is required.
2. Add the selected rendering-layer bit to far background renderers.
3. Add a different rendering-layer bit to midground renderers.
4. Add the third bit to foreground renderers only when a sharp-restore mask is required.
...
| Purpose | Render Layer Mask | Texture Name | Render Queue Upper Bound |
| --- | --- | --- | --- |
| Heavy background blur | `Effect Mask Secondary` | `_BackgroundHeavyBlurMask` | `2499` for opaque 3D, `5000` for sprites/transparent objects |
| Light midground blur | `Effect Mask Primary` | `_MidgroundLightBlurMask` | `2499` for opaque 3D, `5000` for sprites/transparent objects |
| Sharp foreground blocker | A third bit when needed | `_ForegroundSharpMask` | `2499` for opaque 3D, `5000` for sprites/transparent objects |

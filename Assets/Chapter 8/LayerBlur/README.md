...
- `Assets/RenderTextureFeature/Core/MaskedEffect/ObjectMask.mat`: shared material assigned to mask outputs in `ObjectsToRenderTextureFeature`.
- `Assets/RenderTextureFeature/Core/MaskedEffect/InvisibleMaskProxy.shader`: shared invisible camera shader for proxy meshes that define masked effects.
- `Assets/RenderTextureFeature/Core/MaskedEffect/InvisibleMaskProxy.mat`: shared material assigned to invisible proxy renderers.
- `Assets/RenderTextureFeature/Core/RenderingLayerGroup.cs`: reusable hierarchy helper that adds a rendering-layer bit without reserving GameObject layers.
- `FrostedGlassBlur/FrostedGlassBlurRenderer.asset`: dedicated renderer configured for the transparent frosted-glass proxy and `_FrostedGlassBlurMask`.
...
### 1. Choose Rendering-Layer Groups

Use the shared URP rendering layers:

- `Effect Mask Primary`
- `Effect Mask Secondary`

Assign the corresponding bit on each object's `Renderer > Rendering Layer Mask`. Keep the object's GameObject layer available for gameplay, physics, raycasts, and camera culling.

The included scene adds `RenderingLayerGroup` to its foreground and background hierarchy roots, so every child renderer receives the correct bit without editing the environment prefabs.
...
Medium blur output
  ...
  Material: ObjectMask
  Layer Mask: Everything
  Render Layer Mask: Effect Mask Secondary

Heavy blur output
  Texture Name: _HeavyBlurMask
  Material: ObjectMask
  Layer Mask: Everything
  Render Layer Mask: another rendering-layer bit when a third simultaneous group is required
```
...
- `Layer Mask`: `Everything`
- `Render Layer Mask`: the rendering-layer bit assigned to this output's objects
- `Light Mode`: `Standard`
- `Depth`: enabled
...
1. Assign one visible mesh object to each rendering-layer group.
2. Make sure each rendering-layer bit is enabled in its matching mask output.
...
- Add reusable rendering-layer bits to blur-area renderers.
- Change `Renderer.renderingLayerMask` to move objects between blur groups at runtime.
- Use invisible proxy meshes with an effect rendering-layer bit to define custom blur shapes.
...
- One blur layer works but another does not:
  ...
  - Check that the object's renderer has the correct rendering-layer bit.
...
- The blur appears everywhere:
  - Check that each mask output uses `Layer Mask = Everything` and the intended `Render Layer Mask` bit.
  - Check that the mask material is `ObjectMask`.
...
- The blur area is missing:
  - Check the renderer's `Rendering Layer Mask`.
  - Check the output's `Render Layer Mask`.

...
internal sealed class OuterMaskOutlineMaterialSet : IDisposable
{
    public OuterMaskOutlineMaterialSet(Material source)
    {
...
internal sealed class OuterMaskOutlinePass : ScriptableRenderPass
{
    private const int HorizontalMorphologyPass = 0;
    private const int VerticalMorphologyPass = 1;
    private const int CompositePass = 2;

    private static readonly int MorphologyTextureId =
        Shader.PropertyToID("_MorphologyTexture");
    ...
    private readonly MaskedTextureResolver _maskResolver =
        new(nameof(OuterMaskOutlineFeature));

    private string _profilingName;
    private OuterMaskOutlineFeature.Settings _settings;
    private OuterMaskOutlineMaterialSet _materials;
    ...
    public void Setup(
        ...
        OuterMaskOutlineFeature.Settings settings,
        OuterMaskOutlineMaterialSet materials
    )
    {
        _settings = settings;
    ...
    )
    {
        ...
        TextureHandle horizontal = CreateMorphologyTexture(
            renderGraph,
            descriptor,
            "Horizontal"
        );
        TextureHandle morphology = CreateMorphologyTexture(
            renderGraph,
            descriptor,
            "Result"
        );
    ...
    )
    {
        ...
        builder.SetRenderFunc(
            (MorphologyPassData data, RasterGraphContext context) =>
                ExecuteMorphologyPass(data, context)
        );
    }
    ...
    private static void ExecuteMorphologyPass(
        MorphologyPassData data,
